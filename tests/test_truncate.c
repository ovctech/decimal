...
START_TEST(truncate_1) {
  s21_decimal num = {{155, 0, 0, 65536}};
  // s21_decimal num = {{144, 0, 0, 65536}};
  s21_decimal res = {{15, 0, 0, 0}};
  int error = 0;
...
START_TEST(truncate_4) {
  s21_decimal num = {{0, 0, 0, NEGATIVE}};
  s21_decimal res = {{0, 0, 0, NEGATIVE}};
  int error = 0;
  error = s21_truncate(num, &num);
  ck_assert_int_eq(num.bits[0], res.bits[0]);
  ck_assert_int_eq(num.bits[1], res.bits[1]);
...
Suite *suite_truncate(void) {
  ...
  tcase_add_test(tc, truncate_00);
  tcase_add_test(tc, truncate_01);
  tcase_add_test(tc, truncate_02);
  tcase_add_test(tc, truncate_03);
